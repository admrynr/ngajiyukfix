...
<section class="section-b-space">
    <div class="container">
        <div class="checkout-page">
            ...
            <form action="{{ route('cart.order') }}" id="formorder" method="POST">
                @csrf
                            ...
                            <div class="checkout-details">
                                <div class="order-box">
                                    ...
                                    <ul class="qty">
                                        ...
                                        @foreach($cartdetail as $dets)
                                        @php
                                            $totalweight = $totalweight + ($dets->product->weight*$dets->qty);
                                        @endphp
                                        <li>{{$dets->product->product_name}} x {{$dets->qty}}<span>IDR {{number_format($dets->product->final_price * $dets->qty)}}</span></li>
                                        <li>Weight : {{ $dets->product->weight }} x {{ $dets->qty }} = {{ $dets->product->weight*$dets->qty }} gram</li>
                                        @endforeach
                                    </ul>
...
document.getElementById('btnsubmit').onclick = function(){
    ...
    $.ajax({
        url: '{{ route("shop.getmidtranstoken") }}',
        data: formData,
        processData: false,
        contentType: false,
        type: 'POST',
