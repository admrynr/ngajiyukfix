                    ...
                    <div class="row">
                        ...
                        <div class='col-md-6 col-xl-12'>
                            <div class="form-group">
                                ...
                                <img id="fotoimage" src="{{ empty($product->image) ? asset('images/default.png') : asset('images/'.$product->image) }}" style="width:100%;margin-bottom:20px">
                        ...
                        @if(!empty($productalbum))
                            @foreach($productalbum as $keys)
                            <div class="col-md-6 albumimage">
                            <img src="{{ asset('images/'.$keys->src) }}" style="width:100%">
                            <button type="button" class="btn btn-danger btn-sm text-center deleteimage" data-id="{{ $keys->id }}">Delete Image</button>
                            </div>
                            @endforeach
                        @endif
                        <div class="col-md-6 col-xl-12 m-t-10">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class='col-md-12'>
                            <div class="form-group">
                                ...
                                <textarea id="elm1" name="product_description" rows="10">{{ @$product->product_description }}</textarea>
                            </div>
                        </div>
                        <div class='col-md-6'>
                            <div class="form-group">
                                ...
                                <select class="form-control"  name="categories_id" required>
                                    ...
                                    <option value="{{ $key->id }}" {{ $product->categories_id == $key->id ? 'selected' : '' }}>{{ $key->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        {{-- <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control type" name="product_type" id="product_type" required>
                                            ...
                                            <option value="bidding" {{ $product->product_type == 'bidding' ? 'selected' : '' }}>Bidding</option>
                                    </select> 
                                </div>
                            </div> --}}
                    </div>
                    {{-- <div class="row" id="bidding" {{ $product->product_type == 'bidding' ? 'style=display:block' : 'style=display:none' }} >
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <select class="form-control bidding" name="bid_type" id="bid_type">
                                        ...
                                        <option value="low" {{ $product->bid_type == 'low' ? 'selected' : '' }}>Low to High</option>
                                        <option value="high" {{ $product->bid_type == 'high' ? 'selected' : '' }}>High to Low</option>
                                </select> 
                            </div>
                        </div>
                        <div class="col-md-4" >
                            ...
                            <input type="text" name="min_price" id="min_price" value="{{ @$product->min_price }}" class="form-control rupiah" >
                            </div>
                        </div>
                        <div class="col-md-4" >
                            <div class="form-group">
                                <label for="max" class="control-label">Max Price :</label>
                                <input type="text" name="max_price" id="max_price" value="{{ @$product->max_price }}" class="form-control rupiah" >
                            </div>
                        </div>
                        <div class="col-md-4" >
                            <div class="form-group">
                                <label for="scale" class="control-label">Scale :</label>
                                <input type="text" name="scale" id="scale" value="{{ @$product->scale }}" class="form-control nomor" >
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="fixed_price" id="fixed_price" value="{{ @$product->fixed_price }}" class="form-control rupiah">
                            </div>
                        </div>   
                    </div> --}}
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="base_price" id="base_price" value="{{ @$product->base_price }}" class="form-control rupiah" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="final" class="control-label">Final Price :</label>
                                <input type="text" name="final_price" id="final_price" value="{{ @$product->final_price }}" class="form-control rupiah" required>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="sku_code" id="sku_code" class="form-control " value="{{ @$product->sku_code }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="stock" class="control-label">Stock :</label>
                                <input type="number" name="stock" id="stock" min="0" class="form-control" value="{{ @$product->stock }}" required>
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
@section('script-bottom')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $(".rupiah").bind('click keyup change', function(event) {
                    format = $(this).val().replace(/[^,\d]/g, "").toString();
                    // $(".price-value").val(format);
                    $(this).val(formatRupiah(this.value, 'Rp. '));
                });
                $('.rupiah').trigger('change');
                ...
                $('.deleteimage').on('click',function(){
                    var click = confirm("Are you sure to delete image ?");
                    if (click == true) {
                        id = $(this).data('id');
                        $.ajax({
                            url: baseURL+'/admin/product/albumdelete/'+id,
                            type: 'GET',
                            dataType: 'JSON',
                            success: function(data){
                            }
                        });
                        $(this).parent('.albumimage').remove();
                    } else {
